...
def is_garage_open(garage):
    ...
    now = datetime.now()
    current_day = now.strftime("%A")  # Lundi, Mardi, etc.
...
def get_open_status_color(heure_ouverture, heure_fermeture, conge):
    ...
    is_open = is_garage_open_for_horaires(heure_ouverture, heure_fermeture, conge)
    return "green" if is_open else "red"


def is_garage_open_for_horaires(heure_ouverture, heure_fermeture, conge):
    """Vérifie si ouvert à partir des horaires"""
    if not heure_ouverture or not heure_fermeture:
        return False
