...
class _EventHistoriquePageState extends State<EventHistoriquePage>
  ...
  Future<void> _load() async {
    try {
      final data = await EventService.fetchEvents(); // ← fetchEvents pas fetchHistoriqueEvents
      final eventsMap = data['events'] as Map<String, dynamic>? ?? {};
      setState(() {
        _allEvents = eventsMap['all_events'] as List<dynamic>? ?? [];
        _loading = false;
      });
    } catch (e) {
      print("Erreur load: $e");
  ...
  List<dynamic> _filtered() {
    ...
    if (_selectedTab == "Category") {
      base = _allEvents.where((e) => e['event_category'] == 'maintenance').toList();
    } else if (_selectedTab == "By Date") {
      base = _allEvents; // groupByDate va trier par date
    } else {
      base = _allEvents;
    }
  ...
  // ajoute cette variable en haut de la classe
