                      ...
                      child: Column(
                        children: [
                          if (e['event_category'] == 'document') ...[
                            ...
                            _detailRow("Référence", e['reference_unique']),
              ...
              Expanded(
                child: FutureBuilder<Map<String, dynamic>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    final allEvents = (snapshot.data?['events']?['all_events'] as List<dynamic>? ?? []);
                    _eventsMap.clear();
                    ...
                    for (var e in allEvents) {
                      ...
                      if (e['event_category'] == 'document') {
                        dateStr = e['begin_date'];
                      } else {
                        dateStr = e['date_operation'];
                      }
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (_selectedEvents.isEmpty)
                              Text("Aucun événement", style: TextStyle(color: Colors.grey))
                            else
                            ...
                            if (upcomingEvents.isEmpty)
                              _luxuryAnimatedEntry(
                                delay: 0.4,
                                child: Center(child: Text("Aucun événement à venir", style: TextStyle(color: Colors.grey[600]))),
                              )
                            else
                              ...upcomingEvents.asMap().entries.map((entry) {
                                final e = entry.value as Map;
                                final isOverdue = _isOverdue(e);
                                return _luxuryAnimatedEntry(
                                  delay: 0.4 + entry.key * 0.05,
                                  child: _buildEventCard(
                                    e: e,
                                    subtitle: 'Expire: ${_getUpcomingDate(e)}',
                                    tag: isOverdue ? 'Expiré' : 'À venir',
                                    tagColor: isOverdue ? Colors.red : Colors.green,
                                  ),
                                );
                              }),
                            ...
                            if (recentEvents.isEmpty)
                              _luxuryAnimatedEntry(
                                delay: 0.55,
                                child: Center(child: Text("Aucun événement récent", style: TextStyle(color: Colors.grey[600]))),
                              )
                            else
                              ...recentEvents.asMap().entries.map((entry) {
                                final e = entry.value as Map;
                                return _luxuryAnimatedEntry(
                                  delay: 0.55 + entry.key * 0.05,
                                  child: _buildEventCard(
                                    e: e,
                                    subtitle: _getDate(e),
                                  ),
              ...
              child: Column(
                ...
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text(_getTitle(e), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Color(0xFF160078))),
                      if (tag != null)
                        Container(
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(_getLocation(e), style: TextStyle(color: Colors.grey[500], fontSize: 12), overflow: TextOverflow.ellipsis),
                      ),
                    ],
                  ),
                ],
