...
class _TireDetailsPageState extends State<TireDetailsPage>
  ...
  Future<void> _loadTire() async {
    try {
      final data = await DetailPannesService.fetchTire();
      setState(() {
        lastTire = data?["last"];
                      ...
                      child: Row(
                        children: [
                          ...
                          const Column(
                            ...
                            children: [
                              Text("Pneus", style: TextStyle(color: Colors.white, fontSize: 26, fontWeight: FontWeight.bold)),
                          ...
                          Container(
                            ...
                            child: const Icon(Icons.tire_repair, color: Colors.white, size: 24),
                          ),
                        ],
                      ),
                    ),
                    ...
                    child: FadeTransition(
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            if (lastTire != null) _buildLastTireCard(lastTire!),
  ...
  Widget _buildLastTireCard(Map<String, dynamic> data) {
    final garage = data["garage"] ?? {};  // 👈 direct
      ...
      child: Column(
        children: [
          Container(
            ...
            child: Row(
              children: [
                Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), shape: BoxShape.circle), child: const Icon(Icons.tire_repair, color: Colors.white, size: 32)),
                const SizedBox(width: 15),
                Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(data["maintenance_type"] ?? "—", style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
                    Text(_formatDate(data["date_operation"]), style: TextStyle(color: Colors.white.withOpacity(0.8), fontSize: 14)),
                  ]),
                ),
                Container(
          ...
          Padding(
            ...
            child: Column(
              children: [
                Row(children: [
                  Expanded(child: _buildInfoTile(Icons.attach_money, "Coût", "${data["cost"] ?? "—"} TND", const Color(0xFF7226FF))),
                  Expanded(child: _buildInfoTile(Icons.handyman, "Main d'oeuvre", "${data["labor_cost"] ?? "—"} TND", const Color(0xFF160078))),
                ]),
                const SizedBox(height: 15),
                Row(children: [
                  Expanded(child: _buildInfoTile(Icons.timer, "Durée", "${data["actual_repair_time"] ?? "—"} h", Colors.deepPurple)),
                  Expanded(child: _buildInfoTile(Icons.calendar_today, "Date", _formatDate(data["date_operation"]), Colors.purple)),
                ]),
                const SizedBox(height: 15),
                _buildInfoTile(Icons.notes, "Observation", data["observation"] ?? "—", Colors.deepPurple),
                Padding(padding: const EdgeInsets.symmetric(vertical: 18), child: Divider(color: Colors.grey.shade200, thickness: 1.5)),
                Container(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(children: [
                      ...
                      Text(garage["nom"] ?? "—", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF160078))),
                      const Spacer(),
                      Row(children: List.generate(5, (i) => Icon(i < (garage["rating"] ?? 0).round() ? Icons.star : Icons.star_border, color: Colors.amber, size: 16))),
                    ]),
                    const SizedBox(height: 8),
                    Row(children: [Icon(Icons.location_on, color: Colors.grey.shade400, size: 14), const SizedBox(width: 5), Expanded(child: Text(garage["adresse"] ?? "—", style: TextStyle(color: Colors.grey.shade600, fontSize: 13)))]),
                    const SizedBox(height: 5),
                    Row(children: [Icon(Icons.phone, color: Colors.grey.shade400, size: 14), const SizedBox(width: 5), Text(garage["telephone"] ?? "—", style: TextStyle(color: Colors.grey.shade600, fontSize: 13))]),
                  ]),
                ),
              ],
            ),
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text(data["maintenance_type"] ?? "—",  // 👈 direct
                          style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF160078))),
                      Text(_formatDate(data["date_operation"]),  // 👈 direct
                          style: TextStyle(color: Colors.grey.shade500, fontSize: 12)),
                    ],
                  ),
                  const SizedBox(height: 5),
                  Text("Coût: ${data["cost"] ?? "—"} TND  •  ${data["actual_repair_time"] ?? "—"} h",
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(child: Text(garage["nom"] ?? "—",
                          style: TextStyle(color: Colors.grey.shade500, fontSize: 12),
                          overflow: TextOverflow.ellipsis)),
                      Text("${data["cost"] ?? "—"} TND",  // 👈 direct
                          style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Color(0xFF7226FF))),
                    ],
                  ),
                ],
              ),
            ),
            Padding(padding: const EdgeInsets.only(left: 8), child: Icon(Icons.chevron_right, color: Colors.grey.shade300, size: 20)),
          ],
        ),
      ),
    );
          ...
          child: Column(
            children: [
              ...
              Container(
                ...
                child: Row(
                  children: [
                    Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), shape: BoxShape.circle), child: const Icon(Icons.tire_repair, color: Colors.white, size: 24)),
                    ...
                    ElevatedButton.icon(
                      onPressed: () {
                        ...
                        Navigator.of(context).pushAndRemoveUntil(
                          PageRouteBuilder(pageBuilder: (c, a, b) => const PanneDetailsPage(), transitionsBuilder: (c, a, b, child) => SlideTransition(position: Tween<Offset>(begin: const Offset(-1.0, 0.0), end: Offset.zero).animate(CurvedAnimation(parent: a, curve: Curves.easeInOut)), child: child)),
                              (route) => false,
                        );
                      },
                      icon: const Icon(Icons.arrow_back),
                      label: const Text("Retour"),
                      style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF7226FF), foregroundColor: Colors.white, minimumSize: const Size(double.infinity, 50), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20))),
                    ),
                  ],
                ),
              ),
