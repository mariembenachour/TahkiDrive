...
class _DashboardState extends State<Dashboard> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadVehicule() async {
    try {
      ...
      if (dataList.isNotEmpty) {
        ...
        print("✅ FIRST VEHICULE LOADED");
        print("IMAGES RAW: ${_firstVehicule!['images']}");
      }
    } catch (e) {
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          _buildDialogRow(
                            "Coût",
                            "${_batteryData?['cost'] ?? '-'} DT",
                          ),
                          _buildDialogRow(
                            "info",
                            _batteryData?['observation'] ?? '-',
                          ),
                          _buildDialogRow(
                            "Date",
                            _batteryData?['date_operation'] != null
                                ? _batteryData!['date_operation']
                                .toString()
                                .split('T')[0]
                                : '-',
                          ),
                          _buildDialogRow(
                            "Main d'œuvre",
                            "${_batteryData?['labor_cost'] ?? '-'} DT",
                          ),
                          _buildDialogRow(
                            "Temps réparation",
                            "${_batteryData?['actual_repair_time'] ?? '-'} h",
                          ),
                          _buildDialogRow(
                            "Garage",
                            _batteryData?['garage']?['nom'] ?? '-',
                          ),
                        ],
                      ),
                    ),
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          _buildDialogRow(
                            "Coût",
                            "${_oilChangeData?['cost'] ?? '-'} DT",
                          ),
                          _buildDialogRow(
                            "info",
                            _oilChangeData?['observation'] ?? '-',
                          ),
                          _buildDialogRow(
                            "Date",
                            _oilChangeData?['date_operation'] != null
                                ? _oilChangeData!['date_operation']
                                .toString()
                                .split('T')[0]
                                : '-',
                          ),
                          _buildDialogRow(
                            "Main d'œuvre",
                            "${_oilChangeData?['labor_cost'] ?? '-'} DT",
                          ),
                          _buildDialogRow(
                            "Temps réparation",
                            "${_oilChangeData?['actual_repair_time'] ?? '-'} h",
                          ),
                          _buildDialogRow("Garage", _oilChangeData?['garage']?['nom'] ?? '-'),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          _buildDialogRow("Coût", "${data['cost'] ?? '-'} DT"),
                          _buildDialogRow("info", data['observation'] ?? '-'),
                          _buildDialogRow(
                            "Date",
                            data['date_operation'] != null
                                ? data['date_operation'].toString().split('T')[0]
                                : '-',
                          ),
                          _buildDialogRow(
                            "Main d'œuvre",
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(value?.toString() ?? "N/A", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
        ],
      ),
    );
  }
                        ...
                        builder: (context, snapshot) {
                          ...
                          return Column(
                            children: vehicules.map((v) {
                              final insuranceDoc = (v['docs'] as List<dynamic>?)?.firstWhere((d) => d['doc_type'] == "INSURANCE", orElse: () => null);
                              final visitDoc = (v['docs'] as List<dynamic>?)?.firstWhere((d) => d['doc_type'] == "VISIT", orElse: () => null);
                              return _luxuryAnimatedEntry(
                                delay: 0.1,
                                child: GestureDetector(
                                  onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => CarDetailsPage(car: v))),
                                  child: Container(
                                    height: 400,
                                    width: double.infinity,
                                    decoration: BoxDecoration(
                                      ...
                                      children: [
                                        Positioned(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              Text("${v['mark']} ${v['model']}", style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                                              const SizedBox(height: 5),
                                              Text(v['matricule'] ?? "", style: const TextStyle(fontSize: 16, color: Colors.grey)),
                                            ],
                                          ),
                                        ),
                                          ...
                                          builder: (context, value, child) {
                                            ...
                                            try {
                                              if (v['images'] != null && v['images'].toString().isNotEmpty) {
                                                ...
                                                if (decoded.isNotEmpty) imageUrl = 'http://10.0.2.2:8000/${decoded[0]}';
                                              }
                                            } catch (e) {}
                                            return Positioned(
                                              top: 200, bottom: 100, left: value, right: 20,
                                              child: Transform.scale(
                                                scale: 3.8,
                                                child: Image.network(imageUrl, fit: BoxFit.contain, errorBuilder: (context, error, stackTrace) => const Icon(Icons.directions_car, size: 80)),
                                              ),
                                            );
                                          },
                                        ),
                                        Positioned(
                                          ...
                                          child: Row(
                                            children: [
                                              Expanded(child: _buildInfoCard("INSURANCE", insuranceDoc != null ? insuranceDoc['end_date'].split('T')[0] : "N/A")),
                                              const SizedBox(width: 20),
                                              Expanded(child: _buildInfoCard("VISIT", visitDoc != null ? visitDoc['end_date'].split('T')[0] : "N/A")),
                                            ],
                                          ),
                                        ),
                                      ],
                                ...
                                children: [
                                  ...
                                  GestureDetector(
                                    ...
                                    child: _buildExtraLargeStatusCard(
                                      ...
                                      lastChange: _batteryData?['date_operation'] != null
                                          ? _batteryData!['date_operation'].toString().split('T')[0].split('-').reversed.join('/')
                                          : "...",
                                      isDark: true,
                                    ),
                                  ),
                                  ...
                                  GestureDetector(
                                    ...
                                    child: _buildExtraLargeStatusCard(
                                      ...
                                      lastChange: _oilChangeData?['date_operation'] != null
                                          ? _oilChangeData!['date_operation'].toString().split('T')[0].split('-').reversed.join('/')
                                          : "...",
                                      isDark: false,
                                    ),
                                  ),
                                  ...
                                  GestureDetector(
                                    ...
                                    child: _buildExtraLargeStatusCard(
                                      ...
                                      lastChange: _brakeData?['date_operation'] != null
                                          ? _brakeData!['date_operation'].toString().split('T')[0].split('-').reversed.join('/')
                                          : "...",
                                      isDark: true,
                                    ),
                                  ),
                                  ...
                                  GestureDetector(
                                    ...
                                    child: _buildExtraLargeStatusCard(
                                      title: "EMBRAYAGE",
                                      icon: Icons.settings,                                      lastChange: _embrayageData?['date_operation'] != null
                                          ? _embrayageData!['date_operation'].toString().split('T')[0].split('-').reversed.join('/')
                                          : "...",
                                      isDark: true,
                                    ),
                                  ),
  ...
  Widget _buildFloatingButton(IconData icon) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(15), border: Border.all(color: Colors.white.withOpacity(0.2))),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 15),
            const Text("TEMPÉRATURE", style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
