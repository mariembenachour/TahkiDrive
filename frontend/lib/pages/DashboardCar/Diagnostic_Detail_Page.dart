...
class _DiagnosticDetailPageState extends State<DiagnosticDetailPage>
  ...
  Future<void> _loadDiagnostic() async {
    ...
    // ✅ AUTO-PLAY — lit TOUTE la page automatiquement (comme le rapport)
    ...
    if (fullText.isNotEmpty) {
      ...
      NotificationService.speak(fullText).then((_) {
  ...
  Map<String, dynamic> _buildBehaviorDiagnostic() {
    ...
    final data = behaviorData[code] ?? {
      'voice': description,
      'diagnosis': 'Événement de conduite détecté par le système de surveillance.',
      'cause': 'Comportement de conduite nécessitant attention.',
      'action_required': 'Adaptez votre conduite selon les recommandations.',
      'estimated_risk': 'Risque modéré si comportement persistant.',
      'predictions': [
        {'delay': 'Immédiat', 'msg': 'Attention requise', 'sev': 'warning'},
      ],
    };
  ...
  Future<void> _speakParagraph(String key, String text) async {
    ...
    NotificationService.speak(text).then((_) {
