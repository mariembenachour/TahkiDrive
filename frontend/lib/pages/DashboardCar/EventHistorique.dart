...
class _EventHistoriquePageState extends State<EventHistoriquePage>
  ...
  Future<void> _load() async {
    try {
      // ✅ fetchEvents() lit le JWT — pas besoin de passer cin
  ...
  Map<String, List<dynamic>> _groupByDate(List<dynamic> events) {
    ...
    if (_selectedTab == "Category") {
      ...
      for (final e in events) {
        final groupKey = e['event_category'] == 'maintenance' ? "Maintenance" : "Documents";
        g.putIfAbsent(groupKey, () => []).add(e);
      }
      return g;
    }

    // ✅ Tab "All" — grouper par date avec les bonnes clés
    ...
    for (final e in events) {
      DateTime? date;
      try {
        // ✅ CORRIGÉ : utiliser _calendar_date injecté par normalizeEvent()
...
// ✅ CORRIGÉ : bonnes clés pour subtitle
