...
class _NearestMechanicsPageState extends State<NearestMechanicsPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadGarages() async {
    ...
    try {
      ...
      if (_filterByRating) {
        ...
        _allGarages = allGaragesList;
        garages = allGaragesList.where((garage) {
          return GarageService.isOpenNow(garage);
        }).toList();
      } else {
        final allGaragesList = await GarageService.getAllGarages();
        _allGarages = allGaragesList;
  ...
  void _showWeeklyScheduleDialog(BuildContext context, String garageName, Map<String, dynamic> garage) {
    showDialog(
      context: context,
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildScheduleRow("Lundi", garage['heure_ouverture'], garage['heure_fermeture'], garage['conge']),
                _buildScheduleRow("Mardi", garage['heure_ouverture'], garage['heure_fermeture'], garage['conge']),
                _buildScheduleRow("Mercredi", garage['heure_ouverture'], garage['heure_fermeture'], garage['conge']),
                _buildScheduleRow("Jeudi", garage['heure_ouverture'], garage['heure_fermeture'], garage['conge']),
                _buildScheduleRow("Vendredi", garage['heure_ouverture'], garage['heure_fermeture'], garage['conge']),
                _buildScheduleRow("Samedi", garage['heure_ouverture'], garage['heure_fermeture'], garage['conge']),
                _buildScheduleRow("Dimanche", garage['heure_ouverture'], garage['heure_fermeture'], garage['conge']),
                const SizedBox(height: 16),
              ],
            ),
          ),
  ...
  Widget _buildScheduleRow(String day, String? open, String? close, String? conge) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isToday ? Colors.white.withOpacity(0.3) : Colors.white.withOpacity(0.1),
      ...
      child: Row(
        ...
        children: [
          Row(
            children: [
              if (isToday) const Icon(Icons.today, color: Colors.white, size: 16),
          ...
          Text(
            isClosed ? "Fermé" : (open != null && close != null ? "$open - $close" : "Non disponible"),
            style: TextStyle(
              color: isClosed ? Colors.red[300] : Colors.white70,
              fontSize: 13,
            ),
          ),
        ],
  ...
  Widget _buildShopCard({
    ...
    final isOpen = GarageService.isOpenNow(garageData);
    final statusText = GarageService.getOpenStatusText(garageData);
    final statusColor = GarageService.getOpenStatusColor(garageData);
    final openHours = GarageService.getTodayHours(garageData);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          GestureDetector(
            onTap: () {
              _showWeeklyScheduleDialog(context, name, garageData);
            },
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
              decoration: BoxDecoration(
