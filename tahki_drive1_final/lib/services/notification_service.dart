...
class NotificationService {
  ...
  // Fallbacks connus à ignorer — si le backend retourne ça, le vrai diag n'est pas prêt
  ...
  static Future<Map<String, String>> _authHeaders() async {
    ...
    print('>>> [AUTH] token: ${token.isEmpty ? "VIDE!" : token.substring(0, token.length.clamp(0, 20))}...');
    return {
  ...
  static Future<void> _showLocalNotification(RemoteMessage message) async {
    ...
    // ── Lire le mode choisi par le driver ──
    ...
    final int notifId = isMovementAlert
        ? 9999
        : isCombinedAlert
        ? 0
    ...
    await _local.show(
      ...
      NotificationDetails(
        android: AndroidNotificationDetails(
          isReminder ? 'reminders_channel' : 'alerts_channel',
          isReminder ? 'Rappels' : 'Alertes Véhicule',
  ...
  // =========================================
  // FETCH DIAGNOSTIC — ATTEND LE VRAI DIAGNOSTIC IA
  // =========================================
  static Future<Map<String, dynamic>?> fetchDiagnostic(
    ...
    try {
      ...
      // Essaie 5 fois avec 3 secondes entre chaque
      for (int attempt = 0; attempt < 5; attempt++) {
        ...
        // Vérifie que les 3 champs clés sont de vraies données IA
        ...
        print('>>> [DIAG] diagnosis="$diagnosis" → real=$realDiagnosis');
        print('>>> [DIAG] cause="$cause" → real=$realCause');
        print('>>> [DIAG] action="$actionRequired" → real=$realAction');
      ...
      // Dernier recours
      print('>>> [DIAG] ⚠️ 5 tentatives épuisées, retourne le dernier résultat disponible');
  ...
  // DAILY REPORT
  // =========================================
  static Future<Map<String, dynamic>?> getDailyReport(String cin, {String? date}) async {
    try {
  ...
  static Future<void> onBackgroundMessage(RemoteMessage message) async {
    ...
    // ── Lire le mode ──
    final modes  = await NotificationModeService.getModes();
    final String mode = isCritical || isCombined ? 'critique' : 'conduite';
    final bool withTts = modes[mode] == NotificationModeService.SON ||
        modes[mode] == NotificationModeService.SON_VIBRATION;
