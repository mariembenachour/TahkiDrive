...
class _EventHistoriquePageState extends State<EventHistoriquePage>
  ...
  // ── Helper : est-ce une offense ? ──────────────────────────────────────────
  bool _isOffense(Map<String, dynamic> e) =>
  ...
  Map<String, List<dynamic>> _groupByDate(List<dynamic> events) {
    ...
    if (_selectedTab == "Category") {
      ...
      for (final e in events) {
        ...
        } else if (_isOffense(e as Map<String, dynamic>)) {
          groupKey = "Infractions";
        } else {
          groupKey = "Documents";
        }
    ...
    for (final e in events) {
      DateTime? date;
      try {
        final raw = e['_calendar_date'] ?? e['date_reparation'] ?? e['date_panne'] ?? e['date'];
  ...
  IconData _eventIcon(Map<String, dynamic> e) {
    if (e['event_category'] == 'maintenance') {
      ...
      if (t.contains('brake'))        return Icons.disc_full_outlined;
      if (t.contains('battery'))      return Icons.battery_charging_full_rounded;
      if (t.contains('tire'))         return Icons.tire_repair_rounded;
      return Icons.build_circle_outlined;
    }
    // ✅ Offense
    if (_isOffense(e)) return Icons.warning_amber_rounded;
  ...
  Color _eventColor(Map<String, dynamic> e) {
    ...
    // ✅ Offense
    if (_isOffense(e)) return Colors.deepOrange;
  ...
  String _eventTitle(Map<String, dynamic> e) {
    ...
    // ✅ Offense : afficher le vrai type d'infraction
    if (_isOffense(e)) return e['offense_type'] ?? 'Infraction';
  ...
  String _eventDate(Map<String, dynamic> e) {
    // ✅ Offense utilise offense_date
    if (_isOffense(e)) {
      final raw = e['offense_date']?.toString() ?? '';
  ...
  String _eventSubtitle(Map<String, dynamic> e) {
    // ✅ Offense : afficher le montant
    if (_isOffense(e)) {
      final amount = e['paying'];
      if (amount != null) return '$amount TND';
      return '';
    }

    final parts = <String>[];
    if (e['description'] != null && e['description'].toString().isNotEmpty) {
      parts.add(e['description'].toString());
    }
    if (e['cost'] != null) parts.add('${e['cost']} DT');
    if (e['end_date'] != null) {
      final exp = e['end_date'].toString();
      parts.add('Exp: ${exp.length >= 10 ? exp.substring(0, 10) : exp}');
    }
    return parts.join(' • ');
  }
