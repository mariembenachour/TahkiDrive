...
import 'package:tahki_drive1/pages/DashboardCar/ChatPage.dart';
...
class _DailyReportPageState extends State<DailyReportPage>
  ...
  int _lastSpokenIndex = 0; // ← ajouter ici
  ...
  void _toggleMute() {
    if (!_muted) {
      // On arrête → sauvegarde la position
      ...
      // On reprend → depuis _lastSpokenIndex
      setState(() => _muted = false);
      final queue = _buildSpeakQueue();
      if (queue.isNotEmpty && _lastSpokenIndex < queue.length) {
        _speakQueue(queue, _lastSpokenIndex);
      } else {
        _speakAll();
      }
    }
  ...
  Future<void> _loadReport() async {
    ...
    // ← Ajoute ça : parle automatiquement après que les animations démarrent
  ...
  Widget _buildChatOrb() {
    return GestureDetector(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => TakhiChatPage(driverId: widget.cin),
        ),
      ),
      child: AnimatedBuilder(
        ...
        builder: (_, __) => Stack(
          ...
          children: [
            Container(
              width: 70 + 20 * _pulse.value,
              height: 70 + 20 * _pulse.value,
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                color: _neonPurple.withOpacity(0.10 * (1 - _pulse.value)),
              ),
            ),
            Container(
              width: 54, height: 54,
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: _neonPurple.withOpacity(0.6 + 0.3 * _pulse.value),
                  width: 1.5,
                ),
                boxShadow: [
                  BoxShadow(
                    color: _neonPurple.withOpacity(0.35 + 0.2 * _pulse.value),
                    blurRadius: 20,
                  ),
                ],
              ),
                ...
                children: [
                  Row(
                    ...
                    children: List.generate(4, (i) {
                      ...
                      return Container(
                        width: 3,
                        height: heights[i] * (0.7 + 0.3 * _pulse.value),
      ...
      children: List.generate(_bubbleTexts.length, (i) {
        ...
        return Padding(
          ...
          child: FadeTransition(
            ...
            child: SlideTransition(
              position: _bubbleSlide[i],
              child: GestureDetector(          // ← AJOUTE ÇA
                onTap: () => _speakBubble(i),  // ← AJOUTE ÇA
                child: _InsightBubble(
  ...
  Widget _buildBottomActions() {
    return Column(children: [
      ...
      GestureDetector(
        onTap: () {
          NotificationService.stopSpeaking();
          setState(() { _muted = true; _currentBubbleIndex = -1; });
          Navigator.push(
            context,
            PageRouteBuilder(
              pageBuilder: (_, anim, __) => TakhiChatPage(driverId: widget.cin),
          ...
          builder: (_, __) => Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: LinearGradient(
                begin: Alignment.topLeft, end: Alignment.bottomRight,
                colors: [
                  _neonPurple.withOpacity(0.20 + 0.08 * _pulse.value),
                  _neonBlue.withOpacity(0.10),
                ],
              ),
              border: Border.all(
                color: _neonPurple.withOpacity(0.35 + 0.15 * _pulse.value),
              ),
              boxShadow: [
                BoxShadow(
                  color: _neonPurple.withOpacity(0.12 * _pulse.value),
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              SizedBox(
                ...
                child: Row(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.end,
                  children: List.generate(4, (i) {
                    final heights = [6.0, 12.0, 9.0, 5.0];
                    return Container(
                      width: 3,
                      height: heights[i] * (0.6 + 0.4 * _pulse.value),
                      margin: const EdgeInsets.symmetric(horizontal: 1.5),
                      decoration: BoxDecoration(
                        color: _neonPurple.withOpacity(0.7 + 0.3 * _pulse.value),
                        borderRadius: BorderRadius.circular(2),
              ...
              const SizedBox(width: 12),
              Text('PARLER À MA VOITURE',
                  style: TextStyle(
                    color: _labelHi, fontSize: 12,
                    fontWeight: FontWeight.w700, letterSpacing: 1.5,
                    shadows: [Shadow(
                        color: _neonPurple.withOpacity(0.6), blurRadius: 10)],
                  )),
              const SizedBox(width: 12),
              Icon(Icons.arrow_forward_ios_rounded,
                  color: _neonPurple.withOpacity(0.7), size: 12),
            ]),
          ),
        ),
      ),
          ...
          child: BackdropFilter(
            ...
            child: Material(
              ...
              child: InkWell(
                onTap: () {
                  NotificationService.stopSpeaking();
                  setState(() { _currentBubbleIndex = -1; _muted = false; });
                ...
                child: Container(
                  ...
                  child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Icon(Icons.replay_rounded,
                        color: _neonRed.withOpacity(0.9), size: 15),
                    ...
                    Text('REJOUER LE RAPPORT',
                        style: TextStyle(
                          ...
                          shadows: [Shadow(
                              color: _neonRed.withOpacity(0.5), blurRadius: 10)],
                        )),
                  ]),
                ),
              ),
  ...
  void _speakQueue(List<String> texts, int index) {
    if (!mounted || _muted || index >= texts.length) {
    ...
    NotificationService.speak(texts[index]).then((_) {
      if (!mounted || _muted) return;  // ← stop si muted entre-temps
  ...
  void _speakAll() {
    ...
    _lastSpokenIndex = 0; // ← reset position
    final queue = _buildSpeakQueue();
  ...
  void _speakBubble(int index) {
    ...
    if (!_muted && _currentBubbleIndex == index) {
      NotificationService.stopSpeaking();
      setState(() {
        _muted = true;
        _currentBubbleIndex = -1;
    ...
    // Sinon → stop tout, puis joue ce paragraphe
    NotificationService.stopSpeaking();
    setState(() {
      _muted = false;
    ...
    final queue = _buildSpeakQueue();
    final startIndex = index.clamp(0, queue.length - 1);
    _lastSpokenIndex = startIndex;
    Future.delayed(const Duration(milliseconds: 150), () {
      if (mounted) _speakQueue(queue, startIndex);
    });
  }
