...
class NotificationService {
  ...
  static const Set<int> _driverCodes = {0, 1, 2, 3, 4, 9, 10, 11, 12, 14};
  ...
  static int _ttsSentenceIndex = 0;      // ← AJOUTER
  static List<String> _ttsSentences = [];
  ...
  static Future<void> _showLocalNotification(RemoteMessage message) async {
    print(">>> [NOTIF] 🔥 Message reçu: ${message.data}"); // ← ajouter ça

    final data         = message.data;
    final notification = message.notification;
    final bool isMaintenance = data['type'] == 'maintenance';
    ...
    print(">>> [MODES] tous les modes: $modes"); // ← voir ce qui est stocké
    ...
    print(">>> [MODE] modeKey=$modeKey mode=$mode"); // ← voir le mode choisi
    ...
    print(">>> [TTS] withSound=$withSound withTts=$withTts isCritical=$isCritical mustSpeak=${isCritical || isCombinedAlert || isMovementAlert}");
  ...
  static void _navigateFromData(Map<String, dynamic> data) {
    Future.delayed(const Duration(milliseconds: 400), () {
      ...
      if (type == 'panne') {
        ...
        if (reminderType == 'sav_km_reminder') {
          const typeMap = {
            'Vidange':      'Oil Change',
            'Pneu':         'Tire',
            'Frein':        'Brake',
          ...
          final maintenanceTypeRaw = data['maintenance_type']?.toString() ?? '';
          // Si c'est déjà une clé Flutter (vient du backend mappé), l'utiliser directement
          // Sinon chercher dans le map FR
          final maintenanceType = typeMap[maintenanceTypeRaw] ?? maintenanceTypeRaw;

          // ← clés correctes qui correspondent au backend
          final kmSinceRepair = int.tryParse(data['km_parcourus']?.toString() ?? '') ?? 0;
          final kmInterval    = int.tryParse(data['km_interval']?.toString() ?? '') ?? 10000;
          final kmRemaining   = int.tryParse(data['km_restants']?.toString() ?? '') ?? 0;

          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (_) => MaintenanceDiagnosticPage(
                maintenanceType: maintenanceType,
                kmSinceRepair:   kmSinceRepair,
  ...
  static Future<List<dynamic>> getReminders(String cin) async {
    try {
      final headers = await _authHeaders();
      // Uniquement les rappels SAV — driver_reminders n'existe pas en base
      final response = await http.get(
        Uri.parse('$baseUrl/api/agent/sav-reminders'),
        headers: headers,
      );
      if (response.statusCode == 200) {
        return jsonDecode(response.body)['reminders'] ?? [];
      }
      print('>>> [REMINDERS] sav-reminders status: ${response.statusCode}');
      return [];
    } catch (e) {
      print('>>> [REMINDERS] erreur: $e');
      return [];
    }
  }
  ...
  static int getCurrentPosition() => _ttsSentenceIndex;
  static Future<void> speak(String text, {int fromSentence = 0}) async {
    if (_ttsCompleter != null && !_ttsCompleter!.isCompleted) {
      _ttsCompleter!.complete();
    }
    await _tts.stop();
    ...
    _ttsSentenceIndex = fromSentence.clamp(0, _ttsSentences.length - 1);

    for (int i = _ttsSentenceIndex; i < _ttsSentences.length; i++) {
      _ttsSentenceIndex = i;
      final sentence = _ttsSentences[i].trim();
      ...
      // _ttsCompleter == null → stopSpeaking() appelé → arrêt immédiat
      if (_ttsCompleter == null) break;
    }

    // Fin naturelle → reset
    if (_ttsCompleter != null) _ttsSentenceIndex = 0;
  }

  static Future<void> stopSpeaking() async {
    // IMPORTANT : ne pas toucher _ttsSentenceIndex ici
    // L'appelant (toggleVoice) lit getCurrentPosition() AVANT d'appeler stop
    if (_ttsCompleter != null && !_ttsCompleter!.isCompleted) {
      _ttsCompleter!.complete();
    }
    _ttsCompleter = null;
    await _tts.stop();
    // Reset APRES le stop pour que getCurrentPosition() reste valide pendant l'appel
    // Ne reset PAS _ttsSentenceIndex — la page le lit juste avant d'appeler stop
  }
  ...
  static Future<void> _sendTokenToBackend(String token, String cin) async {
    print(">>> [FCM TOKEN] Envoi token: $token");
    try {
      final response = await http.post(
  ...
  static Future<void> onBackgroundMessage(RemoteMessage message) async {
    ...
    // ✅ Créer TOUS les channels d'un coup, toujours
    await androidImpl?.createNotificationChannel(
      const AndroidNotificationChannel(
        'alerts_channel',
        'Alertes Véhicule',
    ...
    if (isDailyReport) {
      final String body = message.notification?.body ??
          data['car_voice'] ??
          "Viens, j'ai des choses à te raconter sur notre journée...";
      final String title = message.notification?.title ?? "🚗 Ton rapport du jour est prêt !";
    ...
    // ✅ Toutes les autres notifs (pannes, alertes, etc.)
    final String title = isCritical
        ? "⚠️ ALERTE CRITIQUE"
        : isCombined
        ? "⚠️ ALERTE COMBINÉE"
        : message.notification?.title ?? "Alerte Véhicule";
    ...
    await local.show(
      ...
      NotificationDetails(
        android: AndroidNotificationDetails(
          'alerts_channel',
          'Alertes Véhicule',
          channelDescription: 'Notifications de sécurité et alertes véhicule',
          importance: Importance.high,
          priority: Priority.high,
          icon: '@mipmap/ic_launcher',
          color: isCritical ? const Color(0xFFFF0000) : const Color(0xFF2979FF),
          playSound: true,
          enableVibration: true,
        ),
      ),
    ...
    print('>>> [BG] ✅ Notif panne/alerte affichée (code=$code)');

    // TTS pour critique/combiné
    if (isCritical || isCombined) {
      final String text = data['car_voice'] ??
          message.notification?.body ??
          (isCombined ? "Alerte combinée dangereuse détectée" : "Alerte véhicule critique");
  ...
  static Future<List<dynamic>> _fetchRawEvents() async {
    try {
      final headers = await _authHeaders();
      final response = await http.get(
        Uri.parse('$baseUrl/api/events/pannes?limit=50'),
      ...
      print('>>> _fetchRawEvents status: ${response.statusCode}');
      if (response.statusCode == 200) {
        final decoded = jsonDecode(response.body);
        final List<dynamic> events = decoded['events'] ?? decoded ?? [];
        return events.map((e) {
      ...
      print('>>> _fetchRawEvents error: ${response.body}');
      return [];
    } catch (e) {
      print('Erreur _fetchRawEvents: $e');
      return [];
    }
  }

  static Future<List<dynamic>> fetchCarEvents(String cin) async {
    final all = await _fetchRawEvents();
    return all.where((e) {
      final code = int.tryParse(e['code']?.toString() ?? '');
      // Voiture = pas de code, ou code qui n'est PAS un code chauffeur
      return code == null || !_driverCodes.contains(code);
    }).toList();
  }

  static Future<List<dynamic>> fetchDriverEvents(String cin) async {
    final all = await _fetchRawEvents();
    return all.where((e) {
      final code = int.tryParse(e['code']?.toString() ?? '');
      // Chauffeur = code strictement dans {9, 10, 11, 12, 14}
      return code != null && _driverCodes.contains(code);
    }).toList();
  }
