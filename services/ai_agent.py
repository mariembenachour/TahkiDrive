...
# Frein, Batterie, Embrayage = détectés comme pannes → pas de suivi km
MAINTENANCE_INTERVALS = {
    "Tire":         10000,   # 10 000 km après dernier entretien
    "Distribution": 10000,   # 10 000 km après dernier entretien
}
...
OIL_CHANGE_KM_INTERVAL   = 10_000   # 10 000 km
OIL_CHANGE_DAYS_INTERVAL = 365      # 1 an

def is_oil_change_due_by_date(date_reparation) -> bool:
    """Retourne True si >= 1 an depuis la dernière vidange."""
    if not date_reparation:
        return False
    if hasattr(date_reparation, 'date'):
        rep_date = date_reparation.date()
    elif isinstance(date_reparation, str):
        try:
            rep_date = datetime.fromisoformat(date_reparation).date()
        except Exception:
            return False
    else:
        rep_date = date_reparation
    return (date.today() - rep_date).days >= OIL_CHANGE_DAYS_INTERVAL
...
def get_km_since_repair(cursor, vehicule_id: str, date_reparation) -> float | None:
    ...
    # ── FIX : pas de données aujourd'hui → device inactif → skip ─────────────
    last_date = row_now["date"]
    if isinstance(last_date, str):
        last_date = datetime.fromisoformat(last_date)
    if last_date.date() < date.today():
        print(f">>> [SAV-KM] Device {vehicule_id} pas de données aujourd'hui (dernière: {last_date.date()}) → skip")
        return None
    # ─────────────────────────────────────────────────────────────────────────
...
# ── SAV KM Reminders ──────────────────────────────────────────────────────────
# ── Caches mémoire ──────────────────────────────────────────────────────────
_sav_km_reminder_sent  = {}   # clé: "[SAV_KM] {id_sav}_{seuil_key}" → dernière valeur odo lue
_fuel_reminder_sent    = {}   # clé: "[FUEL] {vehicule_id}_{seuil_key}" → dernier fuel_pct lu
...
def check_sav_km_reminders():
    ...
    try:
        ...
        for sav in savs:
            mtype_fr      = sav["maintenance_type"]
            mtype_key     = MAINTENANCE_TYPE_MAP.get(mtype_fr)
            date_rep      = sav["date_reparation"]
            vehicule_id   = sav["vehicule_id"]
            driver_id     = sav["driver_id"]
            id_sav        = sav["id_sav"]

            if mtype_key is None:
                print(f">>> [SAV-KM] Type '{mtype_fr}' non géré → skip")
                continue

            mtype = mtype_key

            # ── VIDANGE : 1 an OU 10 000 km ──────────────────────────────────
            if mtype_key == "Oil Change":
                # Vérif date d'abord : >= 1 an → notif directe
                if is_oil_change_due_by_date(date_rep):
                    km_since  = get_km_since_repair(cursor, vehicule_id, date_rep) or 0
                    notif_key = f"[SAV_KM] {id_sav}_urgent"
                    last_km   = _sav_km_reminder_sent.get(notif_key)
                    if last_km is not None and last_km == km_since:
                        print(f">>> [SAV-KM] Vidange date déjà notifiée pour {vehicule_id}, skip")
                        continue
                    rep_date   = date_rep.date() if hasattr(date_rep, 'date') else date_rep
                    days_since = (date.today() - rep_date).days
                    fcm_title  = _get_car_fcm_title("sav_urgent")
                    body = (
                        f"Ça fait {days_since} jours depuis ma dernière vidange ! "
                        f"C'est l'heure, emmène-moi chez le mécanicien 🛢️"
                    )
                    _send_fcm(
                        token=sav["fcm_token"],
                        title=fcm_title,
                        ...
                        data={
                            ...
                            "seuil":               "urgent",
                            "maintenance_type":    mtype_key,
                            "maintenance_type_fr": mtype_fr,
                    ...
                    _sav_km_reminder_sent[notif_key] = km_since
                    print(f">>> [SAV-KM] ✅ Vidange DATE notifiée → {vehicule_id} ({days_since} jours)")
                    continue  # date suffit, pas besoin de vérifier les km

                interval_km = OIL_CHANGE_KM_INTERVAL

            else:
                interval_km = MAINTENANCE_INTERVALS.get(mtype_key)
                if not interval_km:
                    continue

            # ── KM DEPUIS DERNIÈRE RÉPARATION ────────────────────────────────
            ...
            print(f">>> [SAV-KM DEBUG] id_sav={id_sav} | type_fr={mtype_fr} | type_key={mtype_key} | date_rep={date_rep} | km_since={km_since:.0f} | interval={interval_km} | ratio={ratio:.2%}")
            print(f">>> [SAV-KM] {vehicule_id} | {mtype} | {km_since:.0f}/{interval_km} km | {ratio:.0%}")

            # ── RESET DU CACHE si ratio retombe sous 70% ─────────────────────
            if ratio < 0.70:
                for _, sk, _ in REMINDER_THRESHOLDS_SAV:
                    _sav_km_reminder_sent.pop(f"[SAV_KM] {id_sav}_{sk}", None)
                print(f">>> [SAV-KM] Ratio bas ({ratio:.0%}) → cache reset pour {mtype} id={id_sav}")
                continue

            # ── QUEL SEUIL EST ATTEINT ? ──────────────────────────────────────
            seuil_atteint = None
            ...
            notif_key = f"[SAV_KM] {id_sav}_{seuil_key}"

            # ── ANTI-DOUBLON ──────────────────────────────────────────────────
            last_km = _sav_km_reminder_sent.get(notif_key)
            if last_km is not None and last_km == km_since:
                print(f">>> [SAV-KM] Notif '{seuil_key}' déjà envoyée pour {mtype} id={id_sav}, skip")
                continue
            ...
            if ratio >= 1.0:
               body = f"Mon {mtype_fr.lower()} est dépassé de {int(abs(km_restants))} km ! Emmène-moi chez le mécanicien s'il te plaît 🙏"
            elif is_urgent:
               body = f"Il reste seulement {int(km_restants)} km avant mon entretien {mtype_fr.lower()}. On y va bientôt ? 🔧"
            else:
               body = f"Mon {mtype_fr.lower()} approche ({int(km_since)}/{interval_km} km). Un check-up s'impose !"
            ...
            _sav_km_reminder_sent[notif_key] = km_since
            print(f">>> [SAV-KM] ✅ Notif '{seuil_key}' envoyée → {vehicule_id} | {mtype} | {ratio:.0%}")
...
def check_fuel_reminders():
    ...
    try:
        ...
        for drv in drivers:
            ...
            cursor.execute(f"""
                SELECT fuel, date FROM {arch_table}
            ...
            if isinstance(last_date, str):
                last_date = datetime.fromisoformat(last_date)
            if last_date.date() < date.today():
                print(f">>> [FUEL] Device {vehicule_id} pas de données aujourd'hui (dernière: {last_date.date()}) → skip")
                continue
            # ──────────────────────────────────────────────────────────────────
            ...
            # ── RESET DU CACHE si plein ───────────────────────────────────────
            if fuel_pct > 25:
                removed = []
                for sk in ("warning", "critique"):
                    key = f"[FUEL] {vehicule_id}_{sk}"
                    if _fuel_reminder_sent.pop(key, None) is not None:
                        removed.append(sk)
                if removed:
                    print(f">>> [FUEL] Plein détecté ({fuel_pct:.0f}%) → cache reset {removed} pour {vehicule_id}")
                continue
            ...
            # ── CHECK : la valeur fuel a-t-elle changé en base ? ─────────────
            last_fuel = _fuel_reminder_sent.get(notif_key)
            if last_fuel is not None and last_fuel == fuel_current:
                print(f">>> [FUEL] Notif '{seuil_key}' déjà envoyée pour {vehicule_id}, skip")
                continue
            ...
            _fuel_reminder_sent[notif_key] = fuel_current
...
def _save_driver_score(cursor, conn, driver_cin: str, score_data: dict):
    print(f">>> [SCORE] Score calculé pour {driver_cin}: {score_data['global_score']}/100 (non persisté séparément)")
    ...
    try:
        ...
        def get_category_from_code(code: int) -> str:
            mapping = {
                ...
                32: "panne", 33: "panne", 36: "panne", 37: "panne",
                38: "panne", 39: "panne", 40: "panne", 42: "panne",
                44: "panne", 45: "panne", 46: "panne",
                50: "panne", 52: "panne",
                17: "info", 18: "info",
            }
...
def _get_paths_for_report(cursor, driver_cin: str, today) -> list[dict]:
    ...
    all_paths_flat = all_paths["today"] + all_paths["yesterday"] + all_paths["older"]

    today_paths = [
        p for p in all_paths_flat
        if p.begin_path_time and p.begin_path_time.date() == today
    ]
    print(f">>> [PATH-REPORT] {len(today_paths)} trajets trouvés pour {today}")

    arch_table = f"arch_{device_id}"
    nb_trajets  = len(today_paths)
    result      = []
    ...
    for i, path in enumerate(today_paths, 1):
        ...
        # ── Narration ─────────────────────────────────────────────────────────
        if nb_trajets <= 3:
            # Narration normale pour chaque trajet
            ...
            narration = (
                ...
                f"et on est arrivés à {h_end}. "
                f"Chaque kilomètre parcouru ensemble, c'est notre petit moment à nous ! 🚗"
            )
        else:
            # Plus de 3 trajets — narration spéciale premier et dernier
            if i == 1:
                narration = (
                    f"Ce matin à {h_start}, on a commencé notre première aventure ensemble "
                    f"— arrivée à {h_end}. Belle façon de démarrer la journée ! 🌅"
                )
            elif i == nb_trajets:
                narration = (
                    f"Et pour finir en beauté, à {h_start} on a pris notre dernier trajet du jour, "
                    f"terminé à {h_end}. Au total aujourd'hui on a fait {nb_trajets} trajets ensemble — "
                    f"t'as assuré ! 🏁"
                )
            else:
                # Trajets intermédiaires — narration courte
                narration = (
                    f"Trajet {i} : {h_start} → {h_end}. On enchaîne ! 💨"
                )
        ...
        if begin and end:
            try:
                ...
                if gps_rows:
                    ...
                    last = gps_rows[-1]
                    if polyline and (
                        polyline[-1]["lat"] != float(last["latitude"]) or
                        polyline[-1]["lng"] != float(last["longitude"])
                    ):
                        polyline.append({
...
def generate_daily_report(driver_cin: str, target_date=None) -> dict | None:
    ...
    try:
        ...
        score_yesterday = row_yesterday["score_today"] if row_yesterday else None
        ...
        _paths_grouped = get_recent_paths(driver_cin, limit=50, offset=0)
        _all_paths_raw = (
            _paths_grouped["today"] +
            _paths_grouped["yesterday"] +
            _paths_grouped["older"]
        )
        _valid_paths = [p for p in _all_paths_raw if p.begin_path_time]
        paths_date   = max(
            (p.begin_path_time.date() for p in _valid_paths), default=today
        )
        ...
        if paths_today:
            ...
            intro = (
                f"On n'a pas fait de trajet aujourd'hui 🚗💤 "
                f"{vehicule_name} t'attend pour la prochaine sortie !"
            )

        # ── ALERTS SUMMARY ────────────────────────────────────────────────────
        if not paths_today:
            alerts_summary = (
                "Pas de trajet aujourd'hui, donc pas d'alertes. "
                "Je recharge les batteries ! 🔋"
            )
        elif nb == 0:
            alerts_summary = (
                "Aucune alerte aujourd'hui ! "
                "Mes capteurs sont au repos, c'est le paradis 🌴"
            )
        else:
            narrations = [e["narration"] for e in events_narrated]
            alerts_summary = " ".join(narrations[:3])
            if len(narrations) > 3:
                alerts_summary += (
                    f" ... et {len(narrations)-3} autre(s) événement(s) dans la journée."
                )

        # ── SCORE COMMENT ─────────────────────────────────────────────────────
        if diff is None:
            score_comment = (
                f"Score du jour : {score_today}/100. "
                f"Premier bilan disponible, continuons comme ça !"
            )
        elif diff > 0:
            score_comment = (
                f"Score {score_today}/100, en hausse de {diff} points vs hier ! "
                f"Tu carburais bien aujourd'hui 🚀"
            )
        elif diff < 0:
            score_comment = (
                f"Score {score_today}/100, en baisse de {abs(diff)} points vs hier. "
                f"On a connu meilleure route... 📉"
            )
        else:
            score_comment = (
                f"Score {score_today}/100, stable par rapport à hier. "
                f"Régulier comme un moteur bien huilé ! ⚙️"
            )
        ...
        report = {
            ...
            "events_count":    nb,
            "events":          events_with_voice,
            "events_narrated": events_narrated,
            "paths":           paths_today,
