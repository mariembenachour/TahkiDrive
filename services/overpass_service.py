...
# ─── Cache mémoire Groq (TTL 24h) ─────────────────────────────────────────────
_groq_cache: dict[int, dict] = {}
_groq_cache_ts: float = 0.0
CACHE_TTL = 86400  # 24 heures

# ─── Cache résultats finaux (TTL 5min) ────────────────────────────────────────
_results_cache: dict = {}
_RESULTS_CACHE_TTL = 300  # 5 minutes


def _results_cache_get(key: str) -> Optional[list]:
    entry = _results_cache.get(key)
    if entry and (time.time() - entry['ts']) < _RESULTS_CACHE_TTL:
        print(f"[CACHE] ✅ Résultats depuis cache '{key}' ({len(entry['data'])} garages)")
        return entry['data']
    return None


def _results_cache_set(key: str, data: list) -> None:
    _results_cache[key] = {'data': data, 'ts': time.time()}
    print(f"[CACHE] 💾 Sauvegarde cache '{key}' ({len(data)} garages)")
...
async def get_osm_garages(
    ...
    cache_key = f"around_{lat_r}_{lon_r}_{radius_m}_{limit}_{min_rating}"
    ...
    _results_cache_set(cache_key, garages)
    return garages
...
async def warmup_cache() -> None:
    """
    Appelé au démarrage du serveur — précharge le cache national en background.
    L'app reçoit les données instantanément dès le premier appel utilisateur.
    """
    print("[CACHE] 🔥 Warm-up cache national en background...")
    try:
        await get_osm_garages_national(limit=50)
        print("[CACHE] ✅ Warm-up terminé — garages prêts")
    except Exception as e:
        print(f"[CACHE] ⚠️ Warm-up échoué: {e}")