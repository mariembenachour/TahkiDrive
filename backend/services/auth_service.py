...
def get_driver_by_email(email: str) -> CompteDriver | None:
    ...
    try:
        ...
        if not row:
            return None
        return CompteDriver(**row)   # ✅ dict BD → modèle
    except Exception as e:
...
def get_admin_by_email(email: str) -> Administrateur | None:
    ...
    try:
        ...
        if not row:
            return None
        return Administrateur(**row)   # ✅ dict BD → modèle
    except Exception as e:
...
def get_device_by_id(device_id: int) -> Device | None:
    ...
    try:
        ...
        if not row:
            return None
        return Device(**row)   # ✅ dict BD → modèle
    except Exception as e:
...
def get_all_devices() -> list:
    ...
    try:
        ...
        for row in rows:
            try:
                result.append(Device(**row))   # ✅ dict BD → modèle
            except Exception as e:
...
def get_vendor_token(vendor_id: str, token_val: str) -> VendorToken | None:
    ...
    try:
        ...
        if not row:
            return None
        return VendorToken(**row)   # ✅ dict BD → modèle
    except Exception as e:
...
def get_all_drivers() -> list:
    ...
    try:
        ...
        for row in rows:
            try:
                result.append(CompteDriver(**row))   # ✅ dict BD → modèle
            except Exception as e:
...
def get_all_admins() -> list:
    ...
    try:
        ...
        for row in rows:
            try:
                result.append(Administrateur(**row))   # ✅ dict BD → modèle
            except Exception as e:
...
def update_driver_profile(cin: str, data: UpdateProfileRequest) -> bool:
    """Reçoit un modèle UpdateProfileRequest — accès via data.first_name etc."""
    conn = get_connection()
    cursor = conn.cursor()
    try:
        fields = []
        ...
        data_dict = data.dict(exclude_none=True)   # ✅ modèle → dict sans None
