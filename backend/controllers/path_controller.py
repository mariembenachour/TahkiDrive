...
def list_paths(
    limit:  int = Query(default=5,  ge=1, le=50,  description="Nombre de trajets"),
    offset: int = Query(default=0,  ge=0,          description="Pagination offset"),
    cin:    str = Depends(get_current_user),
):
    """
    Retourne les trajets récents du chauffeur connecté.
    - `limit`  : nombre de résultats (1-50, défaut 5)
    - `offset` : pour la pagination (défaut 0)
    """
    paths = get_recent_paths(cin=cin, limit=limit, offset=offset)
    return {
