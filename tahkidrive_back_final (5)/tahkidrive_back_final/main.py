...
load_dotenv()
# ============================================
# INITIALISATION FIREBASE PRO
# ============================================
# Utilise ton fichier existant
if not firebase_admin._apps:
    cred = credentials.Certificate("firebase_credentials.json")
    firebase_admin.initialize_app(cred)
    print(">>> Firebase initialisé avec succès")
else:
    print(">>> Firebase déjà actif (Reload)")
...
async def verify_firebase_token(authorization: str = Header(None)):
    ...
    try:
        # On vérifie le token avec Google
        decoded_token = auth.verify_id_token(token)
        # On vérifie si le mail est validé
...
# ============================================
# ENDPOINT FCM TOKEN (nouveau schéma : compte_driver)
# ============================================
@app.post("/update-fcm-token")
async def update_fcm_token(request: Request):
    """Met à jour le FCM token — nouveau schéma : table compte_driver, clé = cin"""
    try:
        data = await request.json()
        cin       = data.get("cin")        # ← CIN au lieu de driver_id
        fcm_token = data.get("fcm_token")

        if not cin or not fcm_token:
            return {"status": "error", "message": "cin et fcm_token requis"}
        ...
        print(f"✅ Token FCM mis à jour pour cin={cin}")
...
# Ajoute APRÈS app = FastAPI(...) mais AVANT les routes
